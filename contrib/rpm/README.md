...
The RPM spec file provided here is for SteepCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://steepcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[SteepCoin git repository](https://github.com/steepcoin/steepcoin) but are not part of
...
Currently only `Source10` is used, for the example `steepcoin.conf` file.
...
in the steepcoin git repository. At present only an SVG version of the SteepCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the steepcoin
...
needed when 0.12.1 is released, a proper fix is already in the SteepCoin git
master branch.
...
idea to build SteepCoin Core against a static version of that library compiled
...
The `steepcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `steepcoin-server` package. At this point, most RPM based
...
The `steepcoin-server` package should have SELinux support. How to properly do
...
SteepCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/steepcoin/). He has been
packaging SteepCoin for Fedora at least since 2012.
...
4. I renamed the `steepcoin` package that contains the Qt GUI to `steepcoin-core` as
that appears to be how the general population refers to it, in contrast to
`steepcoin-xt` or `steepcoin-classic`. I wanted to make sure the general population
