...
###   steepcoincore.org and steepcoin.org and compares them.
...
WORKINGDIR="/tmp/steepcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://steepcoincore.org"
HOST2="https://steepcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="steepcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'steepcoin-',
...
if [ $? -ne 0 ]; then
   echo "steepcoin.org failed to provide signature file, but steepcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "steepcoin.org and steepcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the SteepCoin Core binary release signing key installed?"
   fi
