#!/bin/bash
# Copyright (c) 2013 The SteepCoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete SteepCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no SteepCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old SteepCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected SteepCoin 0.7 datadir."
    ;;
  3)
    echo "Detected SteepCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected SteepCoin 0.8 datadir."
    ;;
esac
