    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/steepcoin/steepcoin/issues/5716
...
{
    GUIUtil::setClipboard(SteepCoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        //steepTODO maybe add this instead?:
        //nPayFee = (fNewFees ? MIN_TX_FEE : MIN_TX_FEE_PREV7);
...
//            }
            ...
            // steepoin: sub-cent change is moved to fee
...
//            }
    ...
    int nDisplayUnit = SteepCoinUnits::STEEP;
    ...
    l2->setText(SteepCoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(SteepCoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(SteepCoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(SteepCoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const std::pair<QString, std::vector<COutput>>& coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            if(ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show steepcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, SteepCoinUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, SteepCoinUnits::format(nDisplayUnit, nSum));
