...
#include <qt/steepcoingui.h>

#include <qt/steepcoinunits.h>
...
const std::string SteepCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SteepCoinGUI::DEFAULT_WALLET = "~Default";

SteepCoinGUI::SteepCoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(_platformStyle)
{
    ...
    progressBar->setStyleSheet("QProgressBar { background-color: #8C8C8C; text-align: center; color: white; border: 1px solid #4b4b4b; } QProgressBar::chunk { background: #3A4E9B; margin: 0px; }");
...
void SteepCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Steepcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and steepcoin: URIs)"));
...
//    connect(multisigAction, SIGNAL(triggered()), this, SLOT(gotoMultisigPage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Steepcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Steepcoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a steepcoin: URI or payment request"));

    showHelpMessageAction = new QAction(QIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Steepcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void SteepCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void SteepCoinGUI::createToolBars()
{
    if(walletFrame)
    {
...
void SteepCoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool SteepCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool SteepCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SteepCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SteepCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void SteepCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void SteepCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SteepCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SteepCoinGUI::optionsClicked()
{
...
void SteepCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SteepCoinGUI::showDebugWindow()
{
...
void SteepCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SteepCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void SteepCoinGUI::openClicked()
{
...
void SteepCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SteepCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SteepCoinGUI::gotoMultisigPage()
{
    if (walletFrame) walletFrame->gotoMultisigPage();
}

void SteepCoinGUI::gotoMintingPage()
{
    if (walletFrame) walletFrame->gotoMintingPage();
}

void SteepCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SteepCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SteepCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SteepCoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void SteepCoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Steepcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void SteepCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SteepCoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void SteepCoinGUI::updateHeadersSyncProgressLabel()
{
...
void SteepCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void SteepCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Steepcoin"); // default title
...
void SteepCoinGUI::changeEvent(QEvent *e)
{
...
void SteepCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SteepCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SteepCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SteepCoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void SteepCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SteepCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SteepCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SteepCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SteepCoinGUI::setHDStatus(int hdEnabled)
{
...
void SteepCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SteepCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SteepCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SteepCoinGUI::detectShutdown()
{
...
void SteepCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SteepCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void SteepCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SteepCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SteepCoinGUI::subscribeToCoreSignals()
{
...
void SteepCoinGUI::unsubscribeFromCoreSignals()
{
...
void SteepCoinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<SteepCoinUnits::Unit> units = SteepCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SteepCoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(SteepCoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (SteepCoinUnits::Unit u : SteepCoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(SteepCoinUnits::longName(u)), this);
...
{
    setText(SteepCoinUnits::longName(newUnits));
}
