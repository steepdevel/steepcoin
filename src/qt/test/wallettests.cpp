...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount)
{
    ...
    entry->findChild<SteepCoinAmountField*>("payAmount")->setValue(amount);
    uint256 txid;
...
//
//     src/qt/test/test_steepcoin-qt -platform xcb      # Linux
//     src/qt/test/test_steepcoin-qt -platform windows  # Windows
//     src/qt/test/test_steepcoin-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    QString balanceComparison = SteepCoinUnits::formatWithUnit(unit, balance, false, SteepCoinUnits::separatorAlways);
    ...
    // Amount input
    SteepCoinAmountField* amountInput = receiveCoinsDialog.findChild<SteepCoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: steepcoin:")) != -1);
