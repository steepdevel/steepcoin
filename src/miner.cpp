...
// steepcoin: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, CWallet* pwallet, bool* pfPoSCancel)
{
    ...
    // steepcoin: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // steepcoin: there is no point to continue if we failed to create coinstake
    }
...
void PoSMiner(CWallet *pwallet)
{
    ...
    RenameThread("steepcoin-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            // steepcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
...
// steepcoin: stake minter
void MintStake(boost::thread_group& threadGroup)
{
    // steepcoin: mint proof-of-stake blocks in the background
    if (!vpwallets.empty())
