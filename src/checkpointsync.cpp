...
// steepoin: sync-checkpoint master key
...
// steepcoin: synchronized checkpoint (centrally broadcasted)
...
// steepcoin: get last synchronized checkpoint
...
// steepcoin: only descendant of current sync-checkpoint is allowed
...
bool CheckSyncCheckpoint(const uint256& hashBlock, const CBlockIndex* pindexPrev)
{
   ...
   if (pindexPrev->nHeight > 0 && chainActive.Height() == 0) return true;
    ...
    //steepTODO - needs rewrite, because it is very slow during accepting headers
    return true;
...
// steepcoin: reset synchronized checkpoint to last hardened checkpoint
...
// steepcoin: verify signature of sync-checkpoint message
...
// steepcoin: process synchronized checkpoint
