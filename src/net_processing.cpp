...
namespace {
    ...
    /** steepcoin: blocks that are waiting to be processed, the key points to previous CBlockIndex entry */
    struct WaitElement {
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    connman->ForEachNode([this, &pcmpctblock, pindex, &msgMaker, fWitnessEnabled, &hashBlock](CNode* pnode) {
        ...
        if (pnode->GetRecvVersion() < INVALID_CB_NO_BAN_VERSION || pnode->fDisconnect)
            return;
...
void static ProcessGetBlockData(CNode* pfrom, const Consensus::Params& consensusParams, const CInv& inv, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        if (inv.hash == pfrom->hashContinue)
        {
            ...
            // steepcoin: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        // steepcoin: relay sync-checkpoint
        {
            LOCK(cs_main);
    ...
    // steepcoin: set/unset network serialization mode for new clients
    ...
    if (strCommand == NetMsgType::VERACK)
    {
        ...
        if (pfrom->GetRecvVersion() >= SENDHEADERS_VERSION) {
        ...
        if (pfrom->GetRecvVersion() >= SHORT_IDS_BLOCKS_VERSION) {
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // steepcoin: tell downloading node about the latest block if it's
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (!orphanTx.HasWitness() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/steepcoin/steepcoin/issues/8279 for details.
                            assert(recentRejects);
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for (unsigned int n = 0; n < nCount; n++) {
            ...
            // steepcoin: quick check to see if we should ban peers for PoS spam
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
...

        {
            ...
            // steepcoin: store in memory until we can connect it to some chain
        ...
        // steepcoin: accept as many blocks as we possibly can from mapBlocksWait
        while (fContinue) {
            fContinue = false;
            ...
            {
            LOCK(cs_main);
            // steepcoin: try to select next block in a constant time
    ...
    {
        if (pfrom->GetRecvVersion() > BIP0031_VERSION)
        {
            uint64_t nonce = 0;
            vRecv >> nonce;
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (pingSend) {
            ...
            if (pto->GetRecvVersion() > BIP0031_VERSION) {
