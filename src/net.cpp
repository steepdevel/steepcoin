...
// steepcoin: temperature to measure how many PoS headers have been sent by this client
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (SteepCoin users should be well used to small numbers with many decimal places by now :)
    ...
    while (!interruptNet)
    {
        ...
        for (CNode* pnode : vNodesCopy)
        {
            ...
            if (nTime - pnode->nTimeConnected > 60)
            {
                ...
                else if (nTime - pnode->nLastRecv > (pnode->GetRecvVersion() > BIP0031_VERSION ? TIMEOUT_INTERVAL : 90*60))
                {
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Steepcoin " + FormatFullVersion();
