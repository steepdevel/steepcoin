...
#if defined(NDEBUG)
# error "Steepcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Steepcoin Signed Message:\n";
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // steepcoin: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // steepcoin
            undo.nTime = alternate.nTime;           // steepcoin
        } else {
...
void ThreadScriptCheck() {
    RenameThread("steepcoin-scriptch");
...
bool PeercoinContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // steepcoin: compute stake entropy bit for stake modifier
    unsigned int nEntropyBit = GetStakeEntropyBit(block);

    // steepcoin: compute stake modifier
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // steepcoin: track money supply and mint amount info
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // steepcoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // steepcoin: first coinbase output should be empty if proof-of-stake block
...
#ifdef ENABLE_CHECKPOINTS
    // steepcoin: check that the block satisfies synchronized checkpoint
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // steepcoin: we should only accept blocks that can be connected to a prev block with validated PoS
...
#ifdef ENABLE_CHECKPOINTS
    // steepcoin: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // steepcoin: if responsible for sync-checkpoint send it
...
bool CChainState::LoadBlockIndex(const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        // steepcoin: calculate stake modifier checksum
...
#ifdef ENABLE_CHECKPOINTS
    // steepcoin: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams& chainparams)
{
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // steepcoin: txindex is always enabled
...
// steepcoin: total coin age spent in transaction, in the unit of coin-days.
